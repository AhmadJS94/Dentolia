...
function createData(name, address, phone, speciality) {
  return { name, address, phone, speciality };
}
const data = [
  createData('Souheil Haddad', 'Qatana', 12878921, 'all'),
  createData('Zakaria', 'Artouz', 873875423, 'all'),
];
...
export default function LabListGrid() {
  const classes = useRowStyles();
    ...
    <>
      <Box component={Paper} className={classes.header}>
        ...
        {!responsiveSearchBar && (
          ...
          >
            <AddIcon size="small" />
            <span className={classes.addHidden}>Add new lab</span>
          </Button>
        )}
